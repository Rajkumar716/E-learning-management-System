...
<head>
    ...
    <title>STUDENT VIEW TIME TABLE</title>
                            ...
                            <div class="card mb-3">
                                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                                    <div class="info-header">
                                        <h6 class="mb-0 fw-bold ">YOUR TIME TABLE</h6>
                                    </div>
                                ...
                                <div class="card-body">
                                    <table id="myProjectTable" class="table table-hover align-middle mb-0"
                                        ...
                                        <tbody>
                                            <?php
                                               $query="SELECT * FROM `course` WHERE curse_login_username='$_SESSION[coursegmail]' ";
                                               $result=mysqli_query($connection,$query);
                                                 
                                               while($row=mysqli_fetch_assoc($result)){
                                                $batch=$row['course_batch'];
                                                
                                                $_SESSION['course_batch']=$batch;
                                            ...
                                            $query1="SELECT * FROM `time_table` WHERE batch='$_SESSION[course_batch]' AND course_name='$_SESSION[course_name]' AND course_started_year='$_SESSION[course_started_year]'";
                                            $result1=mysqli_query($connection,$query1);
                                                 
                                            while($row1=mysqli_fetch_assoc($result1)){
                                               echo "<tr>";
                                               echo "<td>{$row1['course_name']}</td>";
                                               echo "<td>{$row1['course_started_year']}</td>";
                                               echo "<td>{$row1['batch']}</td>";
                                               echo "<td>{$row1['subject_name']}</td>";
                                               echo "<td>{$row1['prof_name']}</td>";
